...
class RcsInfoFactory(factory.Factory):
    ...
    agencyKey: str = factory.LazyAttribute(lambda n: fake.sentence(nb_words=10)[:20])
    brandKey: str = factory.LazyAttribute(lambda n: fake.sentence(nb_words=10)[:20])
...
class ErrorInfoFactory(factory.Factory):
    ...
    code: str = factory.LazyAttribute(
        lambda n: str(fake.random_element(elements=[x.value[0] for x in errors.ErrorCodeEnum]))
    )
